...
// ============================================
// DentalHire - Company Dashboard (Bilingual)
// ============================================
...
export default function CompanyDashboard() {
    ...
    // Translations specific for Companies
    const t = {
        welcomeBack: language === 'ar' ? 'مرحباً بعودتك،' : 'Welcome back,',
        findPerfect: language === 'ar' ? 'ابحث عن أفضل كوادر المبيعات والتسويق لشركتك' : 'Find the perfect sales and marketing talent for your company',
        findCandidates: language === 'ar' ? 'البحث عن موظفين' : 'Find Employees',
        ...
        candidatesFit: language === 'ar' ? 'المرشحون الأكثر ملاءمة لمتطلبات الشركة' : 'Candidates that best fit company requirements',
        ...
        completeProfile: language === 'ar' ? 'أكمل ملف شركتك لجذب أفضل مندوبي المبيعات والتسويق. الشركات الموثقة موثوقة أكثر.' : 'Complete your company profile to attract top sales and marketing talent. Verified companies are trusted more.',
            ...
            try {
                ...
                const { data: statsData, error: statsError } = await supabase
                    .rpc('get_dashboard_stats', {
                        p_user_id: user.id,
                        p_role: 'clinic' // Using generic stats
                    });
                ...
                // 2. Fetch Top Candidates (Sales, Media, Secretary)
                const { data: recentCVs } = await supabase
                    .from('cvs')
                    .select('*, users!inner(user_type)')
                    .eq('status', 'active')
                    .in('users.user_type', ['sales_rep', 'media', 'secretary']) // STRICT FILTER FOR COMPANIES
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-2">
                    <CardHeader
                        ...
                        action={
                            <Link href="/company/search" className="text-sm text-cyan-600 hover:text-cyan-700 flex items-center gap-1">
                    ...
                    <CardContent>
                        {topCandidates.length > 0 ? (
                            <div className="space-y-4">
                                {topCandidates.map((cv) => (
                                    <div key={cv.id} className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl hover:shadow-md transition-all cursor-pointer">
                                        <div className="w-14 h-14 rounded-full overflow-hidden bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center text-white font-bold text-lg">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1">
                                                <ShoppingBag size={14} /> {t.jobSeeker}
                                            </p>
                            ...
                            <div className="text-center py-8 text-gray-500">
                                <p>{language === 'ar' ? 'لم يتم العثور على مرشحين مناسبين حالياً' : 'No matches found currently'}</p>
                            </div>
                        )}
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="space-y-3">
                                <Link href="/company/search" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <Link href="/company/favorites" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
